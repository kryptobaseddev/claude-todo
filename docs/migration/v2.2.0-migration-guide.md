...
**Expected Output**:
```
Migrating .claude/todo.json from v2.1.0 to v2.2.0...
  Creating backup: .claude/backups/migration/pre-migration-20251215-120000/
  Backup created: .claude/backups/migration/pre-migration-20251215-120000/todo.json
...
6. **Logging**: Record migration in `.claude/.migration.log`
...
PROJECT_NAME=$(jq -r '.project' .claude/todo.json)
...
```bash
jq '.version = "2.2.0"' .claude/todo.json.tmp > .claude/todo.json.new
```
...
jq '.project' .claude/todo.json.new
...
jq '.version' .claude/todo.json  # Should show "2.1.0"
...
claude-todo update T001 --phase setup
claude-todo update T002 --phase core

# Or when creating new tasks
claude-todo add "New feature" --phase core
```
...
**After**:
```bash
PROJECT_NAME=$(jq -r '.project.name' .claude/todo.json)
# Returns: "claude-todo"
```
...
```bash
# Check current state
jq '{version, project}' .claude/todo.json
...
jq '.tasks[] | select(.phase != null and (.phase | IN("setup", "core", "polish", "release") | not))' .claude/todo.json
...
4. **Review Changelog**: Understand what's changing
   ```bash
   cat ~/.claude-todo/CHANGELOG.md | grep "2.2.0" -A 50
   ```
...
2. **Test Operations**: Ensure CLI works correctly
   ```bash
   claude-todo phases
   claude-todo add "Test task" --phase core
   claude-todo update T001 --phase setup
   ```
...
- [ ] Check phases: `claude-todo phases`
- [ ] Test task operations: `claude-todo list`, `claude-todo add "Test"`
...
1. **Check Validation**: `claude-todo validate` often provides specific error messages
2. **Review Logs**: Check `.claude/.migration.log` for migration details
3. **Restore Backup**: When in doubt, rollback and ask for help
4. **File Issue**: Report migration issues on GitHub with:
   - Version information: `claude-todo --version`
   - Migration output
   - Validation errors
   - Contents of `.claude/.migration.log`
