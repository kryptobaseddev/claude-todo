...
| Multiple active | Run `validate-todo.sh --fix` (from project root) |
...
### Quick Commands
```bash
validate-todo.sh           # Validate .claude/todo.json
validate-todo.sh --fix     # Auto-fix simple issues
archive-todo.sh            # Run archive check
archive-todo.sh --dry-run  # Preview archive
log-todo.sh --action X     # Add log entry
```
