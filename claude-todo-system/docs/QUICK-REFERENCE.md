...
| **Status Enum** | Valid status only | "Invalid status: 'done'" |
| **Timestamp Sanity** | Not in future | "created_at in future" |
| **Content Pairing** | Both content & activeForm | "Missing activeForm" |
...
| **todo-archive.schema.json** | Completed tasks | Same as todo.schema.json |
| **todo-config.schema.json** | Configuration | Value ranges, types |
| **todo-log.schema.json** | Change log | Operation types, timestamps |
...
get_config_value "archive.days"      # Get config option
```
...
```json
{
  ...
  "activeForm": "Task description (continuous)",
...
| "Missing activeForm" | Task incomplete | Add activeForm field |
| "Invalid status: 'done'" | Wrong enum value | Use: pending, in_progress, or completed |
